...
int ServerBlock::getSid(void) const
{
	return sid;	
}
...
void ServerBlock::setSid(int id)
{
	this->sid = id;
}
...
bool ServerBlock::findHostName(std::string const& hsotName) const
{
	for(std::vector<std::string>::const_iterator it = serverName.begin(); it != serverName.end(); it++)
	{
		if (*it == hsotName)
			return true;
	}
