...
bool LocationBlock::checkLocation(std::vector<std::string> location)
{
	...
	if (locationName.empty())
		return false;
	if (locationName != "/"  && locationName.at(locationName.length() - 1) == '/')
	{
		locationName.erase(locationName.length() - 1);
	}
...
void LocationBlock::checkLocationBlock(std::string const& root)
{
	// ! add this function instead of upload enable
	if (this->root.empty())
		this->root = root;
	if (this->root.at(this->root.length() - 1) != '/')
		this->root.append("/");
	std::vector<std::string>::iterator it;
	it = find(allowedMethods.begin(), allowedMethods.end(), "POST");
	if (it != allowedMethods.end() && uploadPath.empty())
		throw std::runtime_error("Error: UploadPath not provided!");
}
