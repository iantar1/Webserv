...
bool Config::checkDupServers()
{
	...
	for (size_t i = 0; i < servers.size(); i++)
	{
		...
		{
			server.setSid(servers[i].getSid());
			duplicate = true;
		}
	}
	return duplicate;
...
void Config::pushBlock(std::string context)
{
	if (context == "server")
	{
		...
		if (!checkDupServers())
		{
			...
			tmpServers.push_back(server);
		server = ServerBlock();
	}
	else if (context == "location")
	{
		location.checkLocationBlock(server.getRoot());
...
void Config::parse(void)
{
	...
	for(std::size_t i = 0; i < tmpServers.size(); i++)
	{
		similars[tmpServers[i].getSid()].push_back(tmpServers[i]);
	}
	tmpServers.clear();
	configFile.close();
}